...
<div class="rounded bg-white p-10 w-9/12 mx-auto">
    ...
    <form action="{{ route('admin.vacancies.update', $vacancy) }}" method="POST">
        ...
        <x-admin.textarea name="requirements" label="Requirements"
            :value="is_array($vacancy->requirements) ? implode(', ', $vacancy->requirements) : $vacancy->requirements" />
        <x-admin.textarea name="skills_required" label="Skills Required"
            :value="is_array($vacancy->skills_required) ? implode(', ', $vacancy->skills_required) : $vacancy->skills_required" />
        <x-admin.textarea name="others" label="Others" :value="$vacancy->others" />
        <x-admin.switch-input name="is_active" label="Active" :value="$vacancy->is_active" />
        <x-admin.button type="submit" text="Update Vacancy" class="mt-5" />
        <x-admin.button outline :route="route('admin.vacancies.index')" text="Cancel" />
    </form>
</div>
@endsection