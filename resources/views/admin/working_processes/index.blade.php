...
<x-admin.button :route="route('admin.working-processes.create')" text="Create" />
@endsection
...
<div class="bg-white border border-gray-100 shadow-sm rounded-xl overflow-hidden">
    <table class="w-full text-sm text-gray-700">
        ...
        <tbody class="divide-y divide-gray-100">
            @forelse ($workingProcesses as $workingProcess)
            <tr class="hover:bg-gray-50 transition">
                <td class="px-5 py-3 font-medium">{{ $workingProcess->title }}</td>
                {{-- Truncate long descriptions --}}
                <td class="px-5 py-3 max-w-xs truncate">{{ $workingProcess->description }}</td>
                {{-- Display the icon class/string --}}
                <td class="px-5 py-3 text-gray-500">{{ $workingProcess->icon }}</td>
                <td class="px-5 py-3 text-center">
                    <x-admin.status-badge :is-active="$workingProcess->is_active" />
                </td>
                <td class="px-5 py-3 text-right">
                    <x-admin.actions-dropdown :editUrl="route('admin.working-processes.edit', $workingProcess)"
