...
@section('header-title', 'Edit Client Review')
...
<div class="rounded bg-white p-10 w-9/12 mx-auto">
    <h1 class="text-2xl font-bold mb-4">Edit Client Review</h1>
    ...
    <form action="{{ route('admin.client-reviews.update', $clientReview) }}" method="POST" enctype="multipart/form-data">
        ...
        <x-admin.file-input name="avatar" label="Avatar" :value="$clientReview->avatar" />
        ...
        <x-admin.switch-input name="is_active" label="Active" :value="$clientReview->is_active" />

        <x-admin.button type="submit" text="Update Client Review" class="mt-5" />
